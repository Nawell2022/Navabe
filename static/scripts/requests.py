...
connection = connector.connect(host='localhost', database='NAVABE',
                               user='root', password="Clervie2014!")
...
def send_data() -> list:
    ...
    data = []
    try:
        req = "SELECT * FROM livres limit 150"
        cursor.execute(req)
...
def makeOrder(user: User, transact_id: str, contents: list, amount: float, contentsInfos: list) -> bool:
    ...
    try:
        ...
        request = "CALL commander(%s, %s, %s, %s, @out)"

        cursor.execute(request, (user.getId(), transact_id, json.dumps(contents[0]), amount))
        connection.commit()
...
def checkItem(item: str, qty: int) -> bool:
    """
...
def pass_generator(taille=10) -> str:
    """
